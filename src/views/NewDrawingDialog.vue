...
const dialogVisible = computed({
	get: () => dialogStore.isDrawingDialog && dialogStore.visible,
	set: (val: boolean) => {
		if (!val) {
...
// 存储 typeInfo 数据供创建工程图时使用
const typeInfoDataCache = ref<TypeInfoResponse | null>(null);

// 防止并发重复调用 getTypeInfo
let typeInfoPendingPromise: Promise<TypeInfoResponse> | null = null;
...
const oldClassificationData = ref([
	{
		name: '2D 工程图',
		children: [{ name: '装配图' }, { name: '零件图' }, { name: '布局图' }]
	},
	{
		name: '3D 工程图',
		children: [{ name: '等轴测图' }, { name: '爆炸图' }, { name: '剖视图' }]
	},
	{
		name: '示意图',
		children: [{ name: '原理图' }, { name: '流程图' }, { name: '框图' }]
	}
]);
...
const loadTemplateData = async (spaceValue: string, showLoading = true) => {
	...
	if (typeInfoDataCache.value) {
		processTemplateData(typeInfoDataCache.value);
		processPublicAttributes(typeInfoDataCache.value);
		return;
	}

	// 如果已有请求在进行中，等待它完成并复用结果
	if (typeInfoPendingPromise) {
		try {
			const typeInfoRes = await typeInfoPendingPromise;
	...
	try {
		typeInfoPendingPromise = VplmAPI.getTypeInfo(
			{
				type: 'Drawing',
				preferedType: 'Drawing',
				typeName: 'drawing',
				subTypes: true,
				runUXBL: true,
				metrics: {
	...
	await formRef.value.validate(async valid => {
		if (valid) {
			...
			try {
				...
				// 调试：打印 typeInfo 原始数据
				console.log('【工程图】typeInfo 原始数据 basicAttributes:', typeInfoResult.basicAttributes);
				console.log('【工程图】typeInfo 原始数据 type:', typeInfoResult.type);
				console.log('【工程图】typeInfo 原始数据 integrityToken:', typeInfoResult.integrityToken);
				console.log('【工程图】typeInfo 原始数据 interfaces:', typeInfoResult.interfaces);
				console.log('【工程图】typeInfo 原始数据 subTypes:', typeInfoResult.subTypes);
				...
				// 构建 volatileAttributes（用 formData 中的值覆盖原始值）
				const volatileAttrs = (typeInfoResult.attributes?.volatileAttributes || []).map((attr: any) => {
					const formValue = formData.value[attr.name];
					return {
						...attr,
						value: formValue ?? attr.value ?? '',
					...
					create: [
						{
							...
							attributes: {
								...
								volatileAttributes: volatileAttrs
							},
							basicAttributes
						}
					],
				...
				try {
					...
					console.log('【工程图】getCreateContext 响应:', createCtxResponse);
					if (createCtxResponse?.credentials && createCtxResponse.credentials.length > 0) {
						...
						console.log('【工程图】从 getcreatectx 获取到 securityContext:', securityContext);
					}
				} catch (ctxError) {
					console.warn('【工程图】调用 getcreatectx 失败，将使用备用值:', ctxError);
				}

				// 如果 getcreatectx 没有获取到值，使用备用值
				if (!securityContext) {
					securityContext = baseInfoStore.securityContext || formData.value.collaborativeSpace;
					console.log('【工程图】使用备用 securityContext:', securityContext);
				}

				// 调试：打印完整请求体，便于抓包对比
				console.log('【工程图】createRequest:', JSON.stringify(createRequest, null, 2));

				// 获取勾选的零件物理ID作为 aggregating_context
				const aggregatingContext = [...dialogStore.createContextPhysicalIds];
				console.log('【工程图】aggregating_context:', aggregatingContext);

				// 调用创建工程图接口，传入 aggregating_context 参数建立关联
				const response = await CollabAPI.createDrawing(
					createRequest,
					{
						tenant: 'OnPremise',
						appName: '',
						aggregating_context: aggregatingContext
					},
					{
				...
				console.log('创建工程图响应:', JSON.parse(JSON.stringify(response)));

				// 服务端返回 status: 'error' 时优先识别已知错误码
				if ((response as any)?.status === 'error') {
					const errCode = (response as any).result?.[0]?.errors?.[0]?.errorCode;
					if (errCode === 'ENONEW_ERROR_0002') {
						ElMessage.error('文件名包含无效字符：绘,图');
					} else {
						ElMessage.error(`创建失败${errCode ? '：' + errCode : ''}`);
					}
					return;
				}
				...
				if (response && response.result && response.result.length > 0) {
					...
					if (createdPhysicalId) {
						...
						try {
							...
							console.log('当前 widget 数据:', widgetData);

							// TODO: 后续需要创建 utils/xsceneStoredObjects.ts 并实现 buildMergedXsceneStoredObjects 函数
							// const xsceneStoredObjects = await buildMergedXsceneStoredObjects(widgetData.appId || '', createdPhysicalId);
							// console.log('合并后的 xsceneStoredObjects:', xsceneStoredObjects);

							const widgetRes = await WidgetAPI.editWidgetInstance({
								id: widget.id || '', // 使用实际的 widget.id
								data: {
									x3dSharedId: widgetData.x3dSharedId || '',
									x3dAppId: widgetData.x3dAppId || '',
									x3dPlatformId: widgetData.x3dPlatformId || 'OnPremise',
									appId: widgetData.appId || '',
									token: widgetData.token || '',
									lang: widgetData.lang || 'zh',
									_gpid: widgetData._gpid || 'undefined',
									syncFeatures: widgetData.syncFeatures || '{"Content":true,"ViewPoint":true}',
									x3dLinkability: widgetData.x3dLinkability || {
										open: false,
										appIds: [],
									...
									tenantAware: widgetData.tenantAware || true,
									pad_tenant: widgetData.pad_tenant || 'OnPremise',
									pad_security_ctx: `ctx::${securityContext}`,
									xPref_CREDENTIAL: widgetData.xPref_CREDENTIAL || securityContext,
						...
						if (!isRepeatMode) {
							...
							ElMessage.success('工程图创建成功，可继续创建下一个');
						}
					} else {
						ElMessage.error('工程图创建失败：未获取到工程图ID');
					}
				} else {
					console.error('创建失败，响应异常:', response);
					ElMessage.error('工程图创建失败：响应数据异常');
				}
			} catch (error) {
...
watch(
	() => dialogStore.visible && dialogStore.isDrawingDialog,
	async (newVal, oldVal) => {
...
watch(
	...
	async newVal => {
		if (newVal && dialogStore.isDrawingDialog && dialogStore.visible) {
